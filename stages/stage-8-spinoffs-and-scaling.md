...
```mermaid
flowchart LR
    subgraph TTO["1. Institutional Spin-Out"]
        A["University Lab IP"] --> B["Clean IP Assignment &<br/>Milestone Royalties"]
    end

    subgraph Syndicate["2. Investor Syndicate"]
        B --> C["Lead VC (Pricing)"]
        C --> D["Syndicate Co-Investors<br/>(Follow-on Reserves)"]
    end

    subgraph Market["3. Category Dominance"]
        D --> E["Standards War Execution<br/>(e.g., Qualcomm CDMA)"]
    end
```
