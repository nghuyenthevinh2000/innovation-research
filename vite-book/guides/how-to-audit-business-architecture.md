...
```mermaid
flowchart TD
    BM["BUSINESS MODEL PROMISES"] <--> OM["OPERATING MODEL CAPABILITIES"]
    
    T1["1. Cost vs. Price Test: Does operational delivery cost exceed pricing ceiling?"]
    T2["2. Scalability Test: Does doubling volume require doubling bespoke headcount?"]
    T3["3. Velocity Test: Does sales cycle length align with working capital runway?"]
    T4["4. Defensibility Test: Can competitors copy the offering without our proprietary assets?"]
```
