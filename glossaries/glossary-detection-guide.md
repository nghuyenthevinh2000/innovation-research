...
3. **Automated Auditing:** Executing the CLI coverage script to detect broken `[[wikilinks]]` and unindexed candidate terms in seconds.
...
- **Aliases:** `[[Vault Note Title]]`, `Alternative Name 1`, `Alternative Name 2`
...
- **Innovation Stage:** Stage 1 through Stage 8 where the term is most actively applied.
- **Common Trap / Anti-Pattern:** The specific mistake founders make regarding this concept.
- **Related Terms:** Cross-links to other defined terms using `[[Term Name]]`.
```
...
flowchart TD
    ...
    Q2 -- Yes --> Link["Wrap in [[wikilink]] or verify alias"]
...
1. **Validated Glossary Cross-References:** Total number of wikilinks that successfully resolve to active glossary entries.
2. **Broken / Unindexed Wikilinks (High Priority Gaps):**
   - Shows any `[[Concept]]` referenced in markdown notes that has neither a primary entry nor an alias in `master-glossary.md`.
   - Action: Add the missing entry or add the wikilink target to an existing entry's `- **Aliases:**` list.
...
1. **Locate the Target Section:** Open [`master-glossary.md`](./master-glossary.md) and scroll to the appropriate Innovation Stage section (Stage 1 through Stage 8, or Core Foundations).
2. **Draft the Schema Block:** Populate the entry according to Section 2 above.
3. **Register Aliases:** If the term was referenced by alternative names or vault note filenames (e.g., `[[Seeds - Needs Matching]]` vs `Accelerated Innovation`), include them in `- **Aliases:**`.
